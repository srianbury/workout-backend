...
2. Since the import from mongoexport is broken on mongo express, we need to do a work around to import the data
   ...
   b. `docker exec -it workout-app-local-mongodb /bin/bash`
   c. `cd local-db-data`
...
## Running Queries Inside of the MongoDB Container

1. Get into the mongoDB container

```bash
docker exec -it workout-app-local-mongodb /bin/bash
```

2. Inside of the container, run `mongosh`. Now you can query the database.
